...
discharge_plot_line, = axes.plot(discharge_data[0],
                                 discharge_data[1],
                                 color='red')
...
axes.axvline(x=charge_time,
             ymin=y_limits[0],
             ymax=volt_max / y_limits[1],
             color='green',
             linestyle='dashed')
axes.axhline(y=volt_max,
             xmin=x_limits[0],
             xmax=charge_time / x_limits[1],
             color='green',
             linestyle='dashed')
...
axes.text(x=charge_time + 0.1,
          y=0.05,
          s=str(round(charge_time, 2)),
          fontsize=12)
axes.scatter(x=0.0, y=volt_max, color='green')
axes.text(x=0.1, y=volt_max + 0.05, s=str(round(volt_max, 2)), fontsize=12)
axes.text(x=(charge_time / 2 - 0.8),
          y=volt_max / 2,
          s=("Charge time: " + str(round(charge_time, 2)) + " s"),
          color='blue',
          fontsize=14)
axes.text(x=(charge_time + discharge_time / 2 - 0.8),
          y=volt_max / 2,
          s=("Discharge time: " + str(round(discharge_time, 2)) + " s"),
          color='red',
          fontsize=14)
